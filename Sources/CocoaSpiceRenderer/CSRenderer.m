...
@interface _CSRendererSourceData : NSObject
...
- (nullable instancetype)initWithRenderSource:(id<CSRenderSource>)renderSource atOffset:(CGPoint)offset {
    if (self = [super init]) {
        _viewportOrigin = CGPointMake(renderSource.viewportOrigin.x +
                                      ...
                                      offset.y);
        _scale = renderSource.viewportScale;
        _vertices = renderSource.vertices;
        _numVerticies = renderSource.numVertices;
...
- (nonnull instancetype)initWithMetalKitView:(nonnull MTKView *)mtkView
{
    ...
    if(self)
    {
        ...
        [self _initializeOffScreenRendererWithMTKView:mtkView];
...
- (void)_initializeOffScreenRendererWithMTKView:(MTKView *)mtkView {
    self.drawableSize = mtkView.drawableSize;
    // Set up a texture for rendering to and sampling from
    MTLTextureDescriptor *texDescriptor = [MTLTextureDescriptor new];
    texDescriptor.textureType = MTLTextureType2D;
    texDescriptor.width = self.drawableSize.width;
    texDescriptor.height = self.drawableSize.height;
...
- (void)mtkView:(nonnull MTKView *)view drawableSizeWillChange:(CGSize)size {
    dispatch_async(self.rendererQueue, ^{
        ...
        [self _initializeOffScreenRendererWithMTKView:view];
    });
}
...
- (void)renderSouce:(id<CSRenderSource>)renderSource copyAndDrawWithBuffer:(id<MTLBuffer>)sourceBuffer
  ...
  sourceBytesPerRow:(NSUInteger)sourceBytesPerRow
    ...
    _CSRendererSourceData *cursorData = [[_CSRendererSourceData alloc] initWithRenderSource:renderSource.cursorSource
...
- (void)_renderCommand:(id<MTLCommandBuffer>)commandBuffer
            sourceData:(_CSRendererSourceData *)sourceData
            cursorData:(_CSRendererSourceData *)cursorData
    ...
    NSAssert(sourceData.isVisible, @"Screen should be visible at this point!");
    [self _renderEncoder:renderEncoder
            drawAtOrigin:sourceData.viewportOrigin
                   scale:sourceData.scale
                vertices:sourceData.vertices
            numVerticies:sourceData.numVerticies
                hasAlpha:sourceData.hasAlpha
              isInverted:sourceData.isInverted
                 texture:sourceData.texture];
    
    // Draw cursor
    if (cursorData.isVisible) {
        // Next render the cursor
        [self _renderEncoder:renderEncoder
                drawAtOrigin:cursorData.viewportOrigin
                       scale:cursorData.scale
                    vertices:cursorData.vertices
                numVerticies:cursorData.numVerticies
                    hasAlpha:cursorData.hasAlpha
                  isInverted:cursorData.isInverted
                     texture:cursorData.texture];
    }
